...
public class BotReplyEngine {
    ...
    private static final List<String> FOOD_REPLIES = List.of(
        ...
        "😋 Found restaurants matching your request. Here are the fastest options:",
        "🍽️ I'm checking Zomato, Swiggy, UberEats, and DoorDash for you right now!"
    );

    private static final List<String> TRANSPORT_REPLIES = List.of(
    ...
    private static final List<String> GROCERY_REPLIES = List.of(
    ...
    private static final List<String> UNKNOWN_REPLIES = List.of(
    ...
    public String generateReply(Intent intent, String userMessage) {
        return switch (intent) {
            case FOOD_ORDER             -> FOOD_REPLIES.get(rng.nextInt(FOOD_REPLIES.size()));
            case GROCERY_ORDER          -> GROCERY_REPLIES.get(rng.nextInt(GROCERY_REPLIES.size()));
            case TRANSPORT_BOOK         -> TRANSPORT_REPLIES.get(rng.nextInt(TRANSPORT_REPLIES.size()));
            case SHOPPING_ORDER         -> SHOPPING_REPLIES.get(rng.nextInt(SHOPPING_REPLIES.size()));
            case MULTI_INTENT_RIDE_FOOD -> "🔗 Smart combo mode! I'll sync your ride and food order so everything arrives together.";
            case TRACK_ORDER            -> "📍 To track your order, please share your Order ID and I'll check the status across Zomato, Swiggy, Amazon, Flipkart and more. Example: 'Track order #ZOM123456'";
            case COMPARE                -> "⚖️ I can compare prices, delivery times, and ratings across all integrated apps! Tell me what you're looking for — food, a ride, or a product — and I'll show you the best options side by side.";
            case GREETING               -> GREETING_REPLIES.get(rng.nextInt(GREETING_REPLIES.size()));
            case HELP                   -> "🆘 Here's what I can do for you:\n\n🍕 **Food** — Zomato, Swiggy, UberEats, DoorDash\n🛒 **Groceries** — Blinkit, Zepto, Instacart, Walmart, BigBasket, D-Mart, JioMart\n🚗 **Transport** — Uber, Ola, Lyft, Bolt, Rapido, Yulu\n📦 **Shopping** — Amazon, Flipkart, Meesho, Myntra, eBay\n📍 **Track** — Track any order across all platforms\n⚖️ **Compare** — Compare prices and ETAs side by side\n\nJust tell me what you need in plain English!";
            default                     -> UNKNOWN_REPLIES.get(rng.nextInt(UNKNOWN_REPLIES.size()));
        };
    }
